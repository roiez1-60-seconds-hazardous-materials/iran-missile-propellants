...
export default function Gallery() {
  ...
  return (
    <section id="gallery" className="py-20 px-4 max-w-6xl mx-auto">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white backdrop-blur-sm overflow-hidden mb-8">
        <div className="p-4 border-b border-slate-200 flex items-center justify-between">
          <h3 className="font-bold text-slate-500 text-sm">{h ? 'מצגת מודיעינית — תיק מודיעין טקטי' : 'Intelligence Presentation — Tactical Dossier'}</h3>
        ...
        <div className="relative">
          <AnimatePresence mode="wait">
            <motion.img
              ...
              className="w-full cursor-pointer max-h-[60vh] object-contain"
        ...
        {/* Thumbnail strip */}
        <div className="p-3 flex gap-2 overflow-x-auto bg-slate-100">
          {slides.map((s, i) => (
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
        ...
        <div className="rounded-2xl border border-blue-700/40 bg-blue-50 p-4 hover:border-blue-500/40 transition-all">
          <h3 className="font-bold text-blue-600 mb-3 text-center">🖼️ {h ? 'אינפוגרפיקה: טכנולוגיית ייצור טילים בליסטיים' : 'Infographic: Ballistic Missile Production'}</h3>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="relative max-w-[95vw] max-h-[95vh]" onClick={e => e.stopPropagation()}>
              <button onClick={() => { setFullscreen(false); setShowInfographic(false); }}
                className="absolute top-2 left-2 z-10 bg-black/60 text-white w-10 h-10 rounded-full flex items-center justify-center text-xl hover:bg-black/90">✕</button>
              {showInfographic ? (
                ...
                <>
                  ...
                  <div className="flex justify-between items-center mt-3 px-4">
                    <button onClick={(e) => { e.stopPropagation(); prev(); }}
                      className="px-5 py-2 rounded-lg bg-slate-800 text-slate-500 hover:bg-slate-700 disabled:opacity-30" disabled={current === 0}>
                      {h ? '→ הקודם' : '← Prev'}
                    </button>
                    <span className="text-slate-400 font-mono">{current + 1} / {SLIDE_COUNT}</span>
                    <button onClick={(e) => { e.stopPropagation(); next(); }}
                      className="px-5 py-2 rounded-lg bg-slate-800 text-slate-500 hover:bg-slate-700 disabled:opacity-30" disabled={current === SLIDE_COUNT - 1}>
                      {h ? 'הבא ←' : 'Next →'}
                    </button>
                  </div>
                </>
