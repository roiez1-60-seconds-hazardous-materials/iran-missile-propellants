  ...
  return (
    <section id="home" className="relative flex flex-col items-center justify-center px-4 py-16 md:py-24 overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-64 h-64 bg-red-100/30 rounded-full blur-3xl" />
      </div>
      ...
      <motion.div
        ...
        className="mb-4 px-5 py-1.5 border-2 border-green-600 rounded-lg bg-green-50"
      >
        <span className="text-green-700 font-mono font-bold tracking-[0.3em] text-xs">
          {t('hero.classified')}
        </span>
      </motion.div>
      ...
      <motion.h2
        ...
        className="text-lg md:text-2xl text-slate-500 text-center mb-3 font-bold"
      >
        {t('hero.subtitle')}
      </motion.h2>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}
        ...
        <span className="text-xs text-slate-400 font-mono">{t('hero.date')}</span>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }}
        ...
        {stats.map((s, i) => (
          <motion.div key={i} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 1.1 + i * 0.1, type: 'spring' }}
            className="text-center px-4 py-3 rounded-xl border border-slate-200 bg-white shadow-sm">
            <div className="text-lg mb-0.5">{s.icon}</div>
            <div className="text-2xl md:text-3xl font-black text-blue-600">{t(s.key)}</div>
            <div className="text-[10px] text-slate-400 mt-0.5">{t(s.label)}</div>
          </motion.div>
        ))}
      </motion.div>
      ...
      <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }}
        ...
        className="px-6 py-2.5 rounded-full bg-blue-600 border border-blue-700 text-white font-bold text-sm hover:bg-blue-700 transition-all">
        {t('hero.cta')}
      </motion.button>
    </section>
  );
