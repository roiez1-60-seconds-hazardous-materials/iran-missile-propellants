...
/* ═══ VIEW COUNTER ═══ */
function ViewCounter(){const[v,setV]=useState<number|null>(null);useEffect(()=>{fetch('/api/views').then(r=>r.json()).then(d=>setV(d.views)).catch(()=>{});},[]);if(v===null)return null;return<div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6,marginBottom:10}}><span style={{fontSize:11,color:`#ffffff60`}}>👁️</span><span className="mn" style={{fontSize:12,fontWeight:700,color:P.gold}}>{v.toLocaleString()}</span><span style={{fontSize:10,color:`#ffffff50`}}>views</span></div>;}
...
function Footer({lang}:{lang:string}){const he=lang==="he";return<footer style={{borderTop:`1px solid ${P.gold}40`,padding:"36px 20px",background:P.ink,textAlign:"center"}}>
  ...
  <p style={{fontSize:13,color:`${P.white}cc`,marginBottom:10}}><b style={{color:P.white}}>{he?"רועי צוקרמן":"Roei Zukerman"}</b> — {he?"מומחה לחומ״ס וטב״ק":"HazMat & CBRN Expert"}</p>
  ...
  <div style={{maxWidth:400,margin:"0 auto",padding:"12px 16px",background:`${P.white}08`,borderRadius:8,border:`1px solid ${P.white}15`}}>
    <p style={{fontSize:10,color:P.gold,fontWeight:700,marginBottom:4}}>© 2026 {he?"רועי צוקרמן — מומחה לחומ״ס וטב״ק":"Roei Zukerman — HazMat & CBRN Expert"}</p>
