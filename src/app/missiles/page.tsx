...
function MolecularAnim({lang}:{lang:string}){const he=lang==="he";const[step,setStep]=useState(0);const[phase,setPhase]=useState(0);const[rot,setRot]=useState(0);
useEffect(()=>{const iv=setInterval(()=>setRot(r=>r+0.5),50);return()=>clearInterval(iv);},[]);
useEffect(()=>{setPhase(0);const t1=setTimeout(()=>setPhase(1),400);const t2=setTimeout(()=>setPhase(2),1500);const t3=setTimeout(()=>setPhase(3),3000);return()=>{clearTimeout(t1);clearTimeout(t2);clearTimeout(t3);};},[step]);
const A=({x,y,t,r=12,label,glow}:{x:number;y:number;t:string;r?:number;label?:string;glow?:boolean})=>{const colors:Record<string,string>={N:"#4c6ef5",H:"#e9ecef",O:"#e03131",F:"#37b24d",Pt:"#adb5bd"};const c=colors[t]||"#adb5bd";return<g><defs><radialGradient id={`ag${t}${x}${y}`} cx="35%" cy="25%"><stop offset="0%" stopColor="#fff" stopOpacity="0.8"/><stop offset="40%" stopColor={c}/><stop offset="100%" stopColor={c} stopOpacity="0.7"/></radialGradient></defs>{glow&&<circle cx={x} cy={y} r={r+6} fill="none" stroke={c} strokeWidth="1" opacity="0.4" style={{animation:"apulse 1.2s infinite"}}/>}<circle cx={x} cy={y} r={r} fill={`url(#ag${t}${x}${y})`} stroke={`${c}90`} strokeWidth="0.8" style={{filter:glow?`drop-shadow(0 0 6px ${c})`:"none"}}/><ellipse cx={x-r*0.2} cy={y-r*0.2} rx={r*0.3} ry={r*0.2} fill="rgba(255,255,255,0.45)" transform={`rotate(-25,${x-r*0.2},${y-r*0.2})`}/>{label&&<text x={x} y={y+r+11} textAnchor="middle" fill="#94a3b8" fontSize="7" fontFamily="monospace" fontWeight="600">{label}</text>}</g>;};
const B=({x1,y1,x2,y2,d}:{x1:number;y1:number;x2:number;y2:number;d?:boolean})=>{const nx=(-(y2-y1)/Math.sqrt((x2-x1)**2+(y2-y1)**2))*2;const ny=((x2-x1)/Math.sqrt((x2-x1)**2+(y2-y1)**2))*2;return<g><line x1={x1} y1={y1} x2={x2} y2={y2} stroke="#64748b" strokeWidth="2" strokeLinecap="round"/><line x1={x1} y1={y1} x2={x2} y2={y2} stroke="rgba(255,255,255,0.12)" strokeWidth="0.8"/>{d&&<line x1={x1+nx} y1={y1+ny} x2={x2+nx} y2={y2+ny} stroke="#64748b" strokeWidth="1.5" strokeLinecap="round"/>}</g>;};
const steps=[
{eq:"4NH₃ + 5O₂ → 4NO + 6H₂O",cond:he?"850°C | זרז Pt-Rh 90:10 | 8 atm":"850°C | Pt-Rh 90:10 catalyst | 8 atm",title:he?"חמצון קטליטי":"Catalytic Oxidation",
desc:he?"מולקולות אמוניה (NH₃) מגיבות עם חמצן (O₂) על משטח זרז פלטינה-רודיום ב-850°C. הקשרים N-H נשברים ונוצרים קשרים N=O ו-O-H. תגובה אקזותרמית חזקה.":"Ammonia (NH₃) reacts with oxygen (O₂) on platinum-rhodium catalyst at 850°C. N-H bonds break, N=O and O-H bonds form. Strongly exothermic.",
left:(p:number)=><g style={{opacity:p>=1?1:0,transition:"all 0.7s"}}><B x1={55} y1={50} x2={40} y2={35}/><B x1={55} y1={50} x2={70} y2={35}/><B x1={55} y1={50} x2={55} y2={68}/><A x={55} y={50} t="N" label="N" glow={p===2}/><A x={40} y={35} t="H" r={7}/><A x={70} y={35} t="H" r={7}/><A x={55} y={68} t="H" r={7}/><B x1={140} y1={48} x2={165} y2={48} d/><A x={140} y={48} t="O" label="O₂" glow={p===2}/><A x={165} y={48} t="O" r={11}/>{p===2&&<><rect x={85} y={72} width={50} height={6} rx={3} fill="url(#ptg)" stroke="#adb5bd50"/><text x={110} y={90} textAnchor="middle" fill="#fbbf24" fontSize="6" fontFamily="monospace">Pt-Rh 850°C</text>{[0,1,2,3].map(i=><line key={i} x1={90+i*12} y1={70} x2={90+i*12} y2={64} stroke="#ef4444" strokeWidth="1" opacity="0.6" style={{animation:`hw ${0.6+i*0.1}s infinite`,animationDelay:`${i*0.1}s`}}/>)}</>}</g>,
right:(p:number)=><g style={{opacity:p===3?1:0,transition:"all 0.8s"}}><B x1={50} y1={48} x2={80} y2={48} d/><A x={50} y={48} t="N" label="NO" glow/><A x={80} y={48} t="O"/><B x1={145} y1={42} x2={155} y2={55}/><B x1={165} y1={42} x2={155} y2={55}/><A x={155} y={55} t="O" label="H₂O" glow/><A x={145} y={42} t="H" r={7}/><A x={165} y={42} t="H" r={7}/></g>},
{eq:"2NO + O₂ → 2NO₂",cond:he?"< 150°C | קירור הדרגתי | ספונטני":"< 150°C | Gradual cooling | Spontaneous",title:he?"חמצון משני":"Secondary Oxidation",
desc:he?"גז NO מתקרר ומגיב עם חמצן נוסף מהאוויר ליצירת NO₂ — גז חום-אדום רעיל וכבד. התגובה ספונטנית בטמפרטורה נמוכה ואינה דורשת זרז.":"NO gas cools and reacts with additional oxygen to form NO₂ — toxic reddish-brown heavy gas. Spontaneous at low temperature, no catalyst needed.",
left:(p:number)=><g style={{opacity:p>=1?1:0,transition:"all 0.7s"}}><B x1={40} y1={42} x2={65} y2={42} d/><A x={40} y={42} t="N" glow={p===2}/><A x={65} y={42} t="O"/><B x1={40} y1={65} x2={65} y2={65} d/><A x={40} y={65} t="N" glow={p===2}/><A x={65} y={65} t="O"/><B x1={140} y1={52} x2={165} y2={52} d/><A x={140} y={52} t="O" label="O₂" glow={p===2}/><A x={165} y={52} t="O"/>{p===2&&<text x={110} y={85} textAnchor="middle" fill="#06b6d4" fontSize="7" fontFamily="monospace">❄️ {"<"}150°C</text>}</g>,
right:(p:number)=><g style={{opacity:p===3?1:0,transition:"all 0.8s"}}><B x1={50} y1={35} x2={70} y2={50} d/><B x1={90} y1={35} x2={70} y2={50}/><A x={70} y={50} t="N" label="NO₂" glow/><A x={50} y={35} t="O"/><A x={90} y={35} t="O"/><B x1={130} y1={35} x2={150} y2={50} d/><B x1={170} y1={35} x2={150} y2={50}/><A x={150} y={50} t="N" label="NO₂" glow/><A x={130} y={35} t="O"/><A x={170} y={35} t="O"/><text x={110} y={80} textAnchor="middle" fill="#dc2626" fontSize="6">גז חום-אדום רעיל</text></g>},
{eq:"3NO₂ + H₂O → 2HNO₃ + NO",cond:he?"50-80°C | מגדל ספיגה | לחץ אטמוספרי":"50-80°C | Absorption tower | Atmospheric",title:he?"ספיגה במים":"Water Absorption",
desc:he?"גז NO₂ מוזרם למגדל ספיגה שם הוא נספג במים ויוצר חומצה חנקתית (HNO₃) בריכוז ~68%. זהו המוצר המרכזי של תהליך אוסטוולד — חומר הגלם לכל שרשרת הנשק.":"NO₂ gas is fed into an absorption tower where it dissolves in water to form nitric acid (HNO₃) at ~68% concentration. This is the key Ostwald product — precursor for the entire weapons chain.",
left:(p:number)=><g style={{opacity:p>=1?1:0,transition:"all 0.7s"}}><B x1={40} y1={35} x2={55} y2={48} d/><B x1={70} y1={35} x2={55} y2={48}/><A x={55} y={48} t="N" label="NO₂" glow={p===2}/><A x={40} y={35} t="O"/><A x={70} y={35} t="O"/><B x1={145} y1={40} x2={155} y2={55}/><B x1={165} y1={40} x2={155} y2={55}/><A x={155} y={55} t="O" label="H₂O" glow={p===2}/><A x={145} y={40} t="H" r={7}/><A x={165} y={40} t="H" r={7}/></g>,
right:(p:number)=><g style={{opacity:p===3?1:0,transition:"all 0.8s"}}><B x1={80} y1={30} x2={100} y2={45} d/><B x1={120} y1={30} x2={100} y2={45}/><B x1={100} y1={45} x2={100} y2={62}/><B x1={100} y1={62} x2={115} y2={72}/><A x={100} y={45} t="N" glow/><A x={80} y={30} t="O"/><A x={120} y={30} t="O"/><A x={100} y={62} t="O"/><A x={115} y={72} t="H" r={7}/><text x={100} y={90} textAnchor="middle" fill="#22c55e" fontSize="9" fontWeight="bold" fontFamily="monospace">HNO₃</text></g>},
{eq:"HNO₃ + N₂O₄ + HF → IRFNA",cond:he?"ריכוז > 86% | N₂O₄ 18-27% | HF 0.6%":"Conc. > 86% | N₂O₄ 18-27% | HF 0.6%",title:he?"הכנת IRFNA — מחמצן לטילים":"IRFNA — Missile Oxidizer",
desc:he?"חומצה חנקתית מרוכזת מעל 86%, מוסיפים N₂O₄ (18-27%) ו-HF (0.6% כמעכב קורוזיה). התוצאה: IRFNA — המחמצן בטילי שהאב, גדר, קיאם ועמאד. מגע עם UDMH = הצתה ספונטנית.":"Nitric acid concentrated above 86%, add N₂O₄ (18-27%) and HF (0.6% corrosion inhibitor). Result: IRFNA — oxidizer for Shahab, Ghadr, Qiam and Emad missiles. Contact with UDMH = spontaneous ignition.",
left:(p:number)=><g style={{opacity:p>=1?1:0,transition:"all 0.7s"}}><A x={45} y={45} t="O" r={14} label="HNO₃" glow={p===2}/><A x={120} y={35} t="N" r={12} label="N₂O₄" glow={p===2}/><A x={120} y={65} t="F" r={10} label="HF" glow={p===2}/>{p===2&&<text x={100} y={88} textAnchor="middle" fill="#c8a44e" fontSize="6" fontFamily="monospace">{he?"ריכוז + ערבוב":"Concentrating"}</text>}</g>,
right:(p:number)=><g style={{opacity:p===3?1:0,transition:"all 0.8s"}}><circle cx={105} cy={50} r={30} fill="#ff660012" stroke="#ff6600" strokeWidth="1.5" style={{animation:"apulse 2s infinite"}}/><circle cx={105} cy={50} r={20} fill="#ff660020"/><text x={105} y={46} textAnchor="middle" fill="#ff6600" fontSize="13" fontWeight="900" fontFamily="monospace">IRFNA</text><text x={105} y={58} textAnchor="middle" fill="#ff8800" fontSize="6">{he?"מחמצן טילים":"Missile Oxidizer"}</text><text x={105} y={82} textAnchor="middle" fill="#64748b" fontSize="6">{he?"שהאב • גדר • קיאם • עמאד":"Shahab • Ghadr • Qiam • Emad"}</text></g>},
];const s=steps[step];
return<Sec id="molecular" num="05¾" title={he?"תגובות כימיות — תהליך אוסטוולד":"Chemical Reactions — Ostwald Process"} subtitle={he?"מולקולות תלת-ממדיות • 4 שלבים מ-NH₃ ל-IRFNA":"3D Molecules • 4 steps from NH₃ to IRFNA"} dark>
  <div style={{display:"flex",gap:4,marginBottom:12,justifyContent:"center"}}>{steps.map((_,i)=><button key={i} onClick={()=>setStep(i)} className={step===i?"ta":"ti"} style={{padding:"7px 14px",borderRadius:6,fontSize:11,fontWeight:700,cursor:"pointer"}}>{i+1}. {[he?"חמצון":"Oxid.",he?"קירור":"Cool",he?"ספיגה":"Absorb","IRFNA"][i]}</button>)}</div>
  <div style={{textAlign:"center",marginBottom:8}}><h4 className="sf" style={{fontSize:15,fontWeight:800}}>{s.title}</h4></div>
  <div style={{background:"linear-gradient(180deg,#0f1629,#141c30)",borderRadius:12,border:`1px solid ${P.border}15`,overflow:"hidden",position:"relative",marginBottom:10}}>
    <svg viewBox="0 0 210 100" style={{width:"100%",display:"block"}}>
      <defs><radialGradient id="ptg" cx="50%" cy="30%"><stop offset="0%" stopColor="#dee2e6"/><stop offset="100%" stopColor="#868e96"/></radialGradient></defs>
      <text x={105} y={10} textAnchor="middle" fill={phase===2?"#fbbf24":"#33415540"} fontSize="6" fontFamily="monospace" fontWeight="bold">{phase===1?(he?"מגיבים":"Reactants"):phase===2?`⚡ ${he?"תגובה":"Reaction"} ⚡`:phase===3?`✓ ${he?"תוצרים":"Products"}`:""}</text>
      {phase<3?s.left(phase):s.right(phase)}
    </svg>
    {phase===2&&<div style={{position:"absolute",inset:0,borderRadius:12,background:"radial-gradient(circle,rgba(251,191,36,0.06) 0%,transparent 70%)",animation:"apulse 0.8s infinite"}}/>}
  </div>
  <div dir="ltr" style={{padding:"10px 14px",background:`${P.gold}08`,borderRadius:8,border:`1px solid ${P.gold}15`,textAlign:"center",marginBottom:10}}>
  ...
  <div style={{display:"flex",justifyContent:"center",gap:8,marginTop:12}}><button disabled={step===0} onClick={()=>setStep(s=>s-1)} className={step===0?"ti":"ta"} style={{padding:"7px 14px",borderRadius:6,fontSize:11,fontWeight:700,cursor:step===0?"default":"pointer",opacity:step===0?0.4:1}}>{he?"→ קודם":"→ Prev"}</button><button onClick={()=>setStep(s=>(s+1)%4)} className="ta" style={{padding:"7px 14px",borderRadius:6,fontSize:11,fontWeight:700,cursor:"pointer"}}>{step<3?(he?"הבא ←":"Next ←"):(he?"↻ מההתחלה":"↻ Restart")}</button></div>
</Sec>;}
