...
/* ═══ MOLECULAR REACTION ANIMATION ═══ */
function MolecularAnim({lang}:{lang:string}){const he=lang==="he";const[step,setStep]=useState(0);const[phase,setPhase]=useState(0);const[auto,setAuto]=useState(false);
useEffect(()=>{setPhase(0);const t=[setTimeout(()=>setPhase(1),300),setTimeout(()=>setPhase(2),1600),setTimeout(()=>setPhase(3),3400)];return()=>t.forEach(clearTimeout);},[step]);
useEffect(()=>{if(!auto)return;const iv=setInterval(()=>setStep(s=>(s+1)%4),5500);return()=>clearInterval(iv);},[auto]);
const At=({cx,cy,t,sz=1,gl}:{cx:number;cy:number;t:string;sz?:number;gl?:boolean})=>{const cs:Record<string,string>={N:"#4263eb",H:"#e9ecef",O:"#e03131",F:"#37b24d"};const c=cs[t]||"#adb5bd";const r=({N:13,H:8,O:12,F:10}[t]||12)*sz;const id=`at${t}${Math.round(cx)}${Math.round(cy)}`;
return<g>{gl&&<circle cx={cx} cy={cy} r={r+5} fill={`${c}12`}/>}<ellipse cx={cx+1} cy={cy+r+3} rx={r*0.5} ry={2} fill="rgba(0,0,0,0.15)"/><defs><radialGradient id={id} cx="30%" cy="25%" r="70%"><stop offset="0%" stopColor="#fff" stopOpacity="0.9"/><stop offset="30%" stopColor={c} stopOpacity="0.85"/><stop offset="100%" stopColor={c} stopOpacity="0.6"/></radialGradient></defs><circle cx={cx} cy={cy} r={r} fill={`url(#${id})`} stroke={`${c}50`} strokeWidth="0.6" style={{filter:gl?`drop-shadow(0 0 5px ${c}60)`:"drop-shadow(1px 1px 2px rgba(0,0,0,0.2))"}}/><ellipse cx={cx-r*0.2} cy={cy-r*0.2} rx={r*0.3} ry={r*0.18} fill="rgba(255,255,255,0.45)" transform={`rotate(-20,${cx-r*0.2},${cy-r*0.2})`}/><text x={cx} y={cy+(r>10?4:3)} textAnchor="middle" dominantBaseline="middle" fill={t==="H"?"#495057":"#fff"} fontSize={r>10?10:7} fontWeight="800" fontFamily="Inter,system-ui">{t}</text></g>;};
const Bd=({x1,y1,x2,y2,d,f}:{x1:number;y1:number;x2:number;y2:number;d?:boolean;f?:boolean})=>{const dx=x2-x1,dy=y2-y1,len=Math.sqrt(dx*dx+dy*dy);const nx=(-dy/len)*2,ny=(dx/len)*2;return<g style={{opacity:f?0.3:1,transition:"opacity 0.4s"}}><line x1={x1} y1={y1} x2={x2} y2={y2} stroke="#7a8a9a" strokeWidth={d?1.5:2.5} strokeLinecap="round"/><line x1={x1} y1={y1-0.5} x2={x2} y2={y2-0.5} stroke="rgba(255,255,255,0.1)" strokeWidth="0.6"/>{d&&<line x1={x1+nx} y1={y1+ny} x2={x2+nx} y2={y2+ny} stroke="#7a8a9a" strokeWidth="1.5" strokeLinecap="round"/>}</g>;};
const MG=({tx=0,ty=0,op=1,sc=1,dur="0.8s",del="0s",children}:any)=><g style={{transform:`translate(${tx}px,${ty}px) scale(${sc})`,opacity:op,transition:`all ${dur} cubic-bezier(0.25,0.46,0.45,0.94) ${del}`,transformOrigin:"center"}}>{children}</g>;
const TBar=({temp,x,y}:{temp:number;x:number;y:number})=>{const pct=temp/850;const c=temp>500?"#ef4444":temp>200?"#f59e0b":"#06b6d4";return<g><rect x={x} y={y} width={6} height={55} rx={3} fill="#1e293b" stroke="#33415560" strokeWidth="0.4"/><rect x={x+0.5} y={y+55-pct*53} width={5} height={pct*53} rx={2.5} fill={c} style={{transition:"all 1.5s",filter:`drop-shadow(0 0 2px ${c}50)`}}/><text x={x+3} y={y+62} textAnchor="middle" fill={c} fontSize="5" fontFamily="monospace" fontWeight="700" style={{transition:"all 1s"}}>{temp}°C</text></g>;};
const steps=[
{eq:"4NH₃ + 5O₂ → 4NO + 6H₂O",cond:he?"850°C | זרז Pt-Rh 90:10 | 8 atm | אקזותרמית":"850°C | Pt-Rh 90:10 | 8 atm | Exothermic",title:he?"חמצון קטליטי":"Catalytic Oxidation",
desc:he?"מולקולות NH₃ נעות לעבר O₂ על זרז פלטינה-רודיום ב-850°C. בפגישה — קשרים N-H נשברים, אטומים מתחלפים, ונוצרים N=O (תחמוצת חנקן) ו-O-H (מים).":"NH₃ molecules move toward O₂ on Pt-Rh catalyst at 850°C. On collision — N-H bonds break, atoms rearrange, forming N=O and O-H.",
render:(p:number)=><g>
<MG tx={p===2?55:0} ty={p===2?4:0} op={p===0||p===3?0:p===2?0.3:1} dur={p===2?"1s":"0.6s"}><Bd x1={55} y1={48} x2={42} y2={34} f={p===2}/><Bd x1={55} y1={48} x2={68} y2={34} f={p===2}/><Bd x1={55} y1={48} x2={55} y2={62} f={p===2}/><At cx={55} cy={48} t="N"/><At cx={42} cy={34} t="H" sz={0.7}/><At cx={68} cy={34} t="H" sz={0.7}/><At cx={55} cy={62} t="H" sz={0.7}/></MG>
<MG tx={p===2?-50:0} op={p===0||p===3?0:p===2?0.3:1} dur={p===2?"1s":"0.6s"} del="0.05s"><Bd x1={168} y1={48} x2={192} y2={48} d f={p===2}/><At cx={168} cy={48} t="O"/><At cx={192} cy={48} t="O"/></MG>
{(p===1||p===2)&&<g style={{opacity:0.7}}><rect x={70} y={72} width={80} height={5} rx={2.5} fill="linear-gradient(90deg,#dee2e6,#868e96)" stroke="#adb5bd30"/><text x={110} y={86} textAnchor="middle" fill="#adb5bd" fontSize="6" fontFamily="monospace">Pt-Rh · 850°C</text></g>}
<MG tx={p===3?-55:0} op={p===3?1:0} sc={p===3?1:0.5} dur="0.8s"><Bd x1={100} y1={45} x2={125} y2={45} d/><At cx={100} cy={45} t="N" gl/><At cx={125} cy={45} t="O" gl/></MG>
<MG tx={p===3?50:0} op={p===3?1:0} sc={p===3?1:0.5} dur="0.8s" del="0.12s"><Bd x1={100} y1={38} x2={112} y2={50}/><Bd x1={124} y1={38} x2={112} y2={50}/><At cx={112} cy={50} t="O" gl/><At cx={100} cy={38} t="H" sz={0.7}/><At cx={124} cy={38} t="H" sz={0.7}/></MG>
</g>},
{eq:"2NO + O₂ → 2NO₂",cond:he?"< 150°C | ספונטני בקירור | ללא זרז":"< 150°C | Spontaneous on cooling | No catalyst",title:he?"חמצון בקירור":"Oxidation on Cooling",
desc:he?"ככל שהגז מתקרר מ-850°C לפחות מ-150°C, התגובה ספונטנית: מולקולות NO נעות בעדינות לעבר O₂ ומתחברות. נוצר NO₂ — גז חום-אדום רעיל, כבד מאוויר.":"As gas cools from 850°C to below 150°C, the reaction is spontaneous: NO molecules gently approach O₂ and combine. NO₂ forms — toxic reddish-brown heavy gas.",
render:(p:number)=><g>
<TBar temp={p===0?850:p===1?500:p===2?200:120} x={8} y={10}/>
<MG tx={p===2?45:0} ty={p===2?12:0} op={p===0||p===3?0:p===2?0.3:1} dur="1.2s"><Bd x1={42} y1={38} x2={65} y2={38} d f={p===2}/><At cx={42} cy={38} t="N"/><At cx={65} cy={38} t="O"/></MG>
<MG tx={p===2?45:0} ty={p===2?-12:0} op={p===0||p===3?0:p===2?0.3:1} dur="1.2s" del="0.08s"><Bd x1={42} y1={68} x2={65} y2={68} d f={p===2}/><At cx={42} cy={68} t="N"/><At cx={65} cy={68} t="O"/></MG>
<MG tx={p===2?-48:0} op={p===0||p===3?0:p===2?0.3:1} dur="1.2s" del="0.04s"><Bd x1={170} y1={52} x2={195} y2={52} d f={p===2}/><At cx={170} cy={52} t="O"/><At cx={195} cy={52} t="O"/></MG>
<MG tx={p===3?-58:0} ty={p===3?-6:0} op={p===3?1:0} sc={p===3?1:0.5} dur="0.8s"><Bd x1={98} y1={35} x2={113} y2={48} d/><Bd x1={128} y1={35} x2={113} y2={48}/><At cx={113} cy={48} t="N" gl/><At cx={98} cy={35} t="O"/><At cx={128} cy={35} t="O"/></MG>
<MG tx={p===3?52:0} ty={p===3?5:0} op={p===3?1:0} sc={p===3?1:0.5} dur="0.8s" del="0.12s"><Bd x1={98} y1={35} x2={113} y2={48} d/><Bd x1={128} y1={35} x2={113} y2={48}/><At cx={113} cy={48} t="N" gl/><At cx={98} cy={35} t="O"/><At cx={128} cy={35} t="O"/></MG>
{p===3&&<text x={115} y={78} textAnchor="middle" fill="#dc2626" fontSize="6" fontFamily="'Heebo',sans-serif" fontWeight="600">NO₂ — {he?"גז חום-אדום · צפיפות 3.17":"reddish-brown gas · density 3.17"}</text>}
</g>},
{eq:"3NO₂ + H₂O → 2HNO₃ + NO",cond:he?"50-80°C | מגדל ספיגה | 1 atm":"50-80°C | Absorption tower | 1 atm",title:he?"ספיגה במים":"Absorption in Water",
desc:he?"גז NO₂ נספג במים במגדל ספיגה. המולקולות נפגשות — NO₂ מתפרק, אטומים מתארגנים מחדש לחומצה חנקתית (HNO₃) בריכוז ~68%. חומר הגלם לכל שרשרת הנשק.":"NO₂ dissolves in water in absorption tower. Molecules meet — NO₂ breaks apart, atoms rearrange into nitric acid (HNO₃) at ~68%. Precursor for entire weapons chain.",
render:(p:number)=><g>
<MG tx={p===2?55:0} ty={p===2?6:0} op={p===0||p===3?0:p===2?0.3:1} dur="1s"><Bd x1={35} y1={35} x2={52} y2={48} d f={p===2}/><Bd x1={68} y1={35} x2={52} y2={48} f={p===2}/><At cx={52} cy={48} t="N"/><At cx={35} cy={35} t="O"/><At cx={68} cy={35} t="O"/></MG>
<MG tx={p===2?-48:0} ty={p===2?4:0} op={p===0||p===3?0:p===2?0.3:1} dur="1s" del="0.06s"><Bd x1={168} y1={38} x2={182} y2={50} f={p===2}/><Bd x1={196} y1={38} x2={182} y2={50} f={p===2}/><At cx={182} cy={50} t="O"/><At cx={168} cy={38} t="H" sz={0.7}/><At cx={196} cy={38} t="H" sz={0.7}/></MG>
<MG tx={p===3?-18:0} op={p===3?1:0} sc={p===3?1:0.4} dur="0.9s"><Bd x1={90} y1={25} x2={108} y2={40} d/><Bd x1={126} y1={25} x2={108} y2={40}/><Bd x1={108} y1={40} x2={108} y2={58}/><Bd x1={108} y1={58} x2={122} y2={66}/><At cx={108} cy={40} t="N" gl/><At cx={90} cy={25} t="O"/><At cx={126} cy={25} t="O"/><At cx={108} cy={58} t="O"/><At cx={122} cy={66} t="H" sz={0.7}/><text x={108} y={82} textAnchor="middle" fill="#22c55e" fontSize="8" fontWeight="900" fontFamily="monospace">HNO₃</text></MG>
</g>},
{eq:"HNO₃ + N₂O₄ + HF → IRFNA",cond:he?"ריכוז > 86% | N₂O₄ 18-27% | HF מעכב 0.6%":"Conc. > 86% | N₂O₄ 18-27% | HF inhibitor 0.6%",title:he?"הכנת IRFNA":"IRFNA Production",
desc:he?"שלושה מרכיבים נעים ומתמזגים: HNO₃ מרוכזת + N₂O₄ (מעלה עוצמה) + HF (מעכב קורוזיה). התוצאה: IRFNA — מחמצן טילי שהאב, גדר, קיאם ועמאד. מגע UDMH = הצתה ספונטנית.":"Three components converge and merge: concentrated HNO₃ + N₂O₄ + HF corrosion inhibitor. Result: IRFNA — oxidizer for Shahab, Ghadr, Qiam & Emad. UDMH contact = spontaneous ignition.",
render:(p:number)=><g>
<MG tx={p===2?62:0} ty={p===2?14:0} op={p===0||p===3?0:p===2?0.3:1} dur="1s"><At cx={40} cy={42} t="O" sz={1.2}/><text x={40} y={62} textAnchor="middle" fill="#94a3b8" fontSize="7" fontFamily="monospace">HNO₃</text></MG>
<MG tx={p===2?-10:0} ty={p===2?18:0} op={p===0||p===3?0:p===2?0.3:1} dur="1s" del="0.08s"><At cx={130} cy={30} t="N"/><text x={130} y={50} textAnchor="middle" fill="#94a3b8" fontSize="7" fontFamily="monospace">N₂O₄</text></MG>
<MG tx={p===2?-38:0} ty={p===2?-14:0} op={p===0||p===3?0:p===2?0.3:1} dur="1s" del="0.16s"><At cx={190} cy={65} t="F" sz={0.9}/><text x={190} y={82} textAnchor="middle" fill="#94a3b8" fontSize="7" fontFamily="monospace">HF</text></MG>
...
return<Sec id="molecular" num="10" title={he?"תגובות כימיות — תהליך אוסטוולד":"Chemical Reactions — Ostwald Process"} subtitle={he?"מולקולות מתנגשות • 4 שלבים מ-NH₃ ל-IRFNA":"Molecular collisions • 4 steps from NH₃ to IRFNA"} dark>
  ...
  <div style={{background:"linear-gradient(180deg,#0d1225,#111827)",borderRadius:12,border:`1px solid ${P.white}04`,overflow:"hidden",marginBottom:10}}>
    <svg viewBox="0 0 230 95" style={{width:"100%",display:"block"}}>{s.render(phase)}</svg>
  </div>
  <div dir="ltr" style={{padding:"10px 14px",background:`${P.gold}06`,borderRadius:8,border:`1px solid ${P.gold}10`,textAlign:"center",marginBottom:10}}>
    <div className="mn" style={{fontSize:15,fontWeight:800,color:P.white}}>{s.eq}</div>
    <div className="mn" style={{fontSize:9,color:P.gold,marginTop:3}}>{s.cond}</div>
  </div>
  <p style={{fontSize:12,color:P.muted,lineHeight:1.9,padding:"0 4px"}}>{s.desc}</p>
  <div style={{display:"flex",justifyContent:"center",gap:8,marginTop:12}}>
    <button disabled={step===0} onClick={()=>{setStep(s=>s-1);setAuto(false);}} className={step===0?"ti":"ta"} style={{padding:"7px 14px",borderRadius:6,fontSize:11,fontWeight:700,cursor:step===0?"default":"pointer",opacity:step===0?0.4:1}}>{he?"→ קודם":"→ Prev"}</button>
    <button onClick={()=>setAuto(a=>!a)} className={auto?"ti":"ta"} style={{padding:"7px 14px",borderRadius:6,fontSize:11,fontWeight:700,cursor:"pointer"}}>{auto?(he?"⏸ עצור":"⏸ Stop"):(he?"▶ אוטומטי":"▶ Auto")}</button>
    <button onClick={()=>{setStep(s=>(s+1)%4);setAuto(false);}} className="ta" style={{padding:"7px 14px",borderRadius:6,fontSize:11,fontWeight:700,cursor:"pointer"}}>{step<3?(he?"הבא ←":"Next ←"):(he?"↻ מההתחלה":"↻ Restart")}</button>
  </div>
</Sec>;}
...
function Footer({lang}:{lang:string}){const he=lang==="he";return<footer style={{borderTop:`1px solid ${P.gold}40`,padding:"36px 20px",background:P.ink,textAlign:"center"}}>
  ...
  <div style={{maxWidth:400,margin:"0 auto",padding:"12px 16px",background:`${P.white}08`,borderRadius:8,border:`1px solid ${P.white}15`}}>
    ...
    <p style={{fontSize:9,color:`${P.white}80`,lineHeight:1.6}}>{he?"כל הזכויות שמורות. המידע המופיע באפליקציה זו מבוסס על מקורות פתוחים בלבד ומיועד למטרות מקצועיות והדרכתיות. אין להשתמש במידע זה לכל מטרה אחרת ללא אישור בכתב מהמחבר.":"All rights reserved. Information in this application is based on open sources only and is intended for professional and educational purposes. Do not use this information for any other purpose without written permission from the author."}</p>
  </div>
</footer>;}
